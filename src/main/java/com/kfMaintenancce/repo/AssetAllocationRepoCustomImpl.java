...
public class AssetAllocationRepoCustomImpl implements AssetAllocationRepoCustom {
	@PersistenceContext
	...
	public List<AssetAllocation> getAssetAllocationByLimit(int pageNo, int perPage) {
		...
		try {
			...
			result = (long) entityManager
					.createQuery(
							"SELECT count(*) FROM AssetAllocation a ")
					.getSingleResult();
			q = entityManager.createQuery(
					"select a from  AssetAllocation a ",
	...
	public List<AssetAllocation> getAssetAllocationByLimitAndGroupSearch(GroupSearchDTO groupSearchDTO) {
		...
		String queryStr = "from AssetAllocation a where ";
		int i = 0;
		...
		q = entityManager.createQuery(queryStr, AssetAllocation.class);
