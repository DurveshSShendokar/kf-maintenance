...
public class SparesRepoCustomImpl implements SparesRepoCustom {
	@PersistenceContext
	...
	public List<Spare> getSpareByLimit(int pageNo, int perPage) {
		...
		try {
			...
			result = (long) entityManager
					.createQuery(
							"SELECT count(*) FROM Spare a ")
					.getSingleResult();
			q = entityManager.createQuery(
					"select a from  Spare a ",
					Spare.class);
	...
	public List<Spare> getSpareByLimitAndGroupSearch(GroupSearchDTO groupSearchDTO) {
		...
		String queryStr = "from Spare a where ";
		int i = 0;
		...
		q = entityManager.createQuery(queryStr, Spare.class);
