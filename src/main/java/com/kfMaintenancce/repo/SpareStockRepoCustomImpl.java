...
public class SpareStockRepoCustomImpl implements SpareStockRepoCustom {
	@PersistenceContext
	...
	public List<SpareStock> getSpareStockByLimit(int pageNo, int perPage) {
		...
		try {
			...
			result = (long) entityManager
					.createQuery(
							"SELECT count(*) FROM SpareStock a ")
					.getSingleResult();
			q = entityManager.createQuery(
					"select a from  SpareStock a ",
					SpareStock.class);
			...
			List<SpareStock> list = q.getResultList();
			return list;
	...
	public List<SpareStock> getSpareStockByLimitAndGroupSearch(GroupSearchDTO groupSearchDTO) {
		...
		String queryStr = "from SpareStock a where ";
		int i = 0;
		...
		q = entityManager.createQuery(queryStr, SpareStock.class);
