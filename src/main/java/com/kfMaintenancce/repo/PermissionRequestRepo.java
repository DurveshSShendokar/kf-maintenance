...
import com.kfMaintenancce.model.PermissionRequest;
...
public interface PermissionRequestRepo extends JpaRepository<PermissionRequest, Integer>{
	@Query("From PermissionRequest p where p.permissions.permissionsId=?1 and p.permissionAction.permissionAsactionId=?2 and p.userInfo.id=?3")
	...
	@Query("From PermissionRequest p where p.approved=1 and p.userInfo.id=?1")
	List<PermissionRequest> getApprovedPermissionRequestByUser(int userid);
	
	@Query("From PermissionRequest p where p.approved=1 and p.userInfo.id=?1 and p.permissions.permissionsId=?2")
	List<PermissionRequest> getApprovedPermissionRequestByUserAndPermission(int userid, int permissionsId);
	
	
	@Query("From PermissionRequest p where p.approved=1 and p.userInfo.id=?1 and p.permissions.permissionsId=?2 and p.permissionAction.actionName=?3  ")
