...
public class AssetInventoryCustomeRepoImpl implements AssetInventoryCustomeRepo {
	...
	public List<AssetInventory> getAssetInventoryByLimit(int pageNo, int perPage) {
		...
		try {
			...
			result = (long) entityManager
					.createQuery(
							"SELECT count(*) FROM AssetInventory a ")
					.getSingleResult();
			q = entityManager.createQuery(
					"select a from  AssetInventory a ",
	...
	public List<AssetInventory> getAssetInventoryByLimitAndGroupSearch(GroupSearchDTO groupSearchDTO) {
		...
		String queryStr = "from AssetInventory a where ";
		int i = 0;
		...
		q = entityManager.createQuery(queryStr, AssetInventory.class);
